...
public class Reply extends Command {
    ...
    public boolean execute(@NotNull final CommandSender sender, @NotNull final String command, final String[] args) {
        if (!(sender instanceof final Player mcplayer)) {
            plugin.getServer().getConsoleSender().sendMessage(LocalizedMessage.COMMAND_MUST_BE_RUN_BY_PLAYER.toString());
            return true;
        }

        final MineverseChatPlayer mcp = MineverseChatAPI.getOnlineMineverseChatPlayer(mcplayer);
        if (mcp == null) {
            return true;
        }
        ...
        final MineverseChatPlayer player = MineverseChatAPI.getOnlineMineverseChatPlayer(mcp.getReplyPlayer());
        if (player == null) {
            mcp.getPlayer().sendMessage(LocalizedMessage.NO_PLAYER_TO_REPLY_TO.toString());
            return true;
        }
        ...
        send = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(mcp.getPlayer(), Objects.requireNonNull(plugin.getConfig().getString("replyformatfrom")).replaceAll("sender_", "")));
        echo = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(mcp.getPlayer(), Objects.requireNonNull(plugin.getConfig().getString("replyformatto")).replaceAll("sender_", "")));
        spy = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(mcp.getPlayer(), Objects.requireNonNull(plugin.getConfig().getString("replyformatspy")).replaceAll("sender_", "")));

        send = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(player.getPlayer(), send.replace("receiver_", ""))) + msg;
        echo = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(player.getPlayer(), echo.replace("receiver_", ""))) + msg;
        spy = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(player.getPlayer(), spy.replace("receiver_", ""))) + msg;
    ...
    private void sendBungeeCordReply(final MineverseChatPlayer mcp, final String @NotNull [] args) {
        ...
        final String send = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(mcp.getPlayer(), Objects.requireNonNull(plugin.getConfig().getString("replyformatfrom")).replaceAll("sender_", "")));
        final String echo = Format.FormatStringAll(PlaceholderAPI.setBracketPlaceholders(mcp.getPlayer(), Objects.requireNonNull(plugin.getConfig().getString("replyformatto")).replaceAll("sender_", "")));
